...
export const ResizablePanels = component$<ResizablePanelsProps>(
  ({
    ...
    const mousePosition = useStore({ x: 50, y: 50 });
    ...
    useOn(
      ...
      $((event: Event) => {
        ...
        storageKey &&
          localStorage.setItem(storageKey, JSON.stringify(mousePosition));
      }),
    );
    useOn(
      "touchmove",
      $((event: TouchEvent) => {
        ...
        isMouseDown.value &&
          (mousePosition.y =
            ((event.touches[0].clientY - rect.top) / rect.height) * 100);
        min && mousePosition.x < min && (mousePosition.x = min);
        max && mousePosition.x > max && (mousePosition.x = max);
        min && mousePosition.y < min && (mousePosition.y = min);
        max && mousePosition.y > max && (mousePosition.y = max);
    ...
    return (
      <div className={className} ref={thisRef}>
        ...
        >
          ...
          <div className={"cc-resizablePanels-secondPanel"}></div>
        </div>
      </div>
    );
  },
